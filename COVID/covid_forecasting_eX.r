...
for (i in 1:te_size) {
    ...
    exog_data <- window(exog_ts, start = as.Date(i + zero_reference), end = as.Date(tr_size - 1 + zero_reference + i), frequency = 1)
    # Retraining after kt samples
    if (i == 1) {
        arima_model <- Arima(y = model_data, order = c(p, d, q), seasonal = list(order = c(PP, DD, QQ), period = s), xreg=lag(exog_data, 7), method = "CSS")
    } else { 
        if ((i %% kt) == 0) { #if iteration mod retrain frequency is 0, then it is time to retrain the model with new training data
            ...
            arima_model <- Arima(y = model_data,xreg=lag(exog_data, 7), model = arima_model)
        }
    }
    # Once we have the model set: either with new data or with new data and retrained get next "horizon" number of forecasts.
    future <- forecast(arima_model,xreg=lag(exog_data, 7), h = horizons, level = 0)
