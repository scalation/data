#install.packages("forecast")  #Unccomment and run this for the first time R is setup on your machine
library(forecast)
...
for (i in 1:horizons) {
  smape_scores[i, 1] <- mean(abs((test_data - forecast_matrix[, i]) / (test_data + forecast_matrix[, i]))) * 200 #smape formula
